...
void mp3_play_music(I2S_HandleTypeDef* hi2s, I2C_HandleTypeDef *hi2c, char* file_name){
	...
	while(mp3_play_flag){
		if(file_read_flag){
			...
			err = MP3Decode(mp3decoder, &read_ptr, &bytes_left, &(mp3_out_buf[mp3_buf_pos]), 0); 
			frames++;
			if(err != ERR_MP3_NONE){
				switch (err)
				{
				...
				if(outputSamps > 0){
					if(mp3frameinfo.nChans == 1){
						...
						{
							mp3_out_buf[mp3_buf_pos + i * 2] = mp3_out_buf[mp3_buf_pos + i];
							mp3_out_buf[mp3_buf_pos + i * 2 + 1] = mp3_out_buf[mp3_buf_pos + i];
						}
						outputSamps *= 2;
					}
				}
				...
				if(mp3frameinfo.samprate != mp3_player.sample_rate){
					...
					if(mp3_player.sample_rate >= I2S_AUDIOFREQ_DEFAULT){
						...
						coded_i2s_set_up(hi2s, hi2c,mp3_player.sample_rate, I2S_DATAFORMAT_16B);
						HAL_I2S_Transmit_DMA(hi2s, (mp3_out_buf + mp3_buf_pos), outputSamps);
