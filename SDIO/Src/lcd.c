...
void LCD_Write_Cmd(uint16_t usCmd) { *(__IO uint16_t*)(FSMC_Addr_LCD_CMD) = usCmd; }

void LCD_Write_Data(uint16_t usData) { *(__IO uint16_t*)(FSMC_Addr_LCD_DATA) = usData; }
...
void LCD_DrawEllipse(uint16_t usC, uint16_t usP, uint16_t SR, uint16_t LR, uint16_t usColor) {
	...
	d1 = (LR * LR) - (SR * SR * LR) + (0.25 * SR * SR);
	dx = 2 * LR * LR * x;
	dy = 2 * SR * SR * y;

	// For region 1
	while (dx < dy) {
		// Print points based on 4-way symmetLR
	...
	d2 = ((LR * LR) * ((x + 0.5) * (x + 0.5))) + ((SR * SR) * ((y - 1) * (y - 1))) - (SR * SR * LR * LR);

	// Plotting points of region 2
	while (y >= 0) {
		// printing points based on 4-way symmetLR
		LCD_DrawDot(x + usC, y + usP,usColor);
		LCD_DrawDot(-x + usC, y + usP,usColor);
		LCD_DrawDot(x + usC, -y + usP,usColor);
		LCD_DrawDot(-x + usC, -y + usP,usColor);
...
const uint8_t name_array[] =
{

};

void LCD_chinese_name(uint16_t usC, uint16_t usP,uint16_t usColor, uint16_t us_backgrdColor){
	
	LCD_OpenWindow(usC, usP, 32, 32);
	LCD_Write_Cmd(CMD_SetPixel);
	
	for (int i = 0; i < 32 * 32 / 8; i ++){
		uint8_t temp = name_array[i];
		for (int j = 0; j < 8; j++){
			if (temp & 0x80)
					LCD_Write_Data(usColor);

				else
					LCD_Write_Data(us_backgrdColor);
